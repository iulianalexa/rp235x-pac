...
#[doc = "Field `SUSPENDED` reader - Bus in suspended state. Valid for device. Device will go into suspend if neither Keep Alive / SOF frames are enabled."]
pub type SUSPENDED_R = crate::BitReader;
#[doc = "Field `SUSPENDED` writer - Bus in suspended state. Valid for device. Device will go into suspend if neither Keep Alive / SOF frames are enabled."]
...
pub type RX_SHORT_PACKET_W<'a, REG> = crate::BitWriter1C<'a, REG>;
...
#[doc = "Field `ENDPOINT_ERROR` reader - An endpoint has encountered an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
pub type ENDPOINT_ERROR_R = crate::BitReader;
#[doc = "Field `ENDPOINT_ERROR` writer - An endpoint has encountered an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
pub type ENDPOINT_ERROR_W<'a, REG> = crate::BitWriter1C<'a, REG>;
...
impl R {
    ...
    #[doc = "Bit 4 - Bus in suspended state. Valid for device. Device will go into suspend if neither Keep Alive / SOF frames are enabled."]
    #[inline(always)]
    ...
    #[doc = "Bit 12 - Device or Host has received a short packet. This is when the data received is less than configured in the buffer control register. Device: If using double buffered mode on device the buffer select will not be toggled after writing status back to the buffer control register. This is to prevent any further transactions on that endpoint until the user has reset the buffer control registers. Host: the current transfer will be stopped early."]
    #[inline(always)]
    pub fn rx_short_packet(&self) -> RX_SHORT_PACKET_R {
        RX_SHORT_PACKET_R::new(((self.bits >> 12) & 1) != 0)
    }
    ...
    #[doc = "Bit 23 - An endpoint has encountered an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
    #[inline(always)]
    pub fn endpoint_error(&self) -> ENDPOINT_ERROR_R {
        ENDPOINT_ERROR_R::new(((self.bits >> 23) & 1) != 0)
    }
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn rx_short_packet(&mut self) -> RX_SHORT_PACKET_W<SIE_STATUS_SPEC> {
        RX_SHORT_PACKET_W::new(self, 12)
    }
    #[doc = "Bit 17 - Device: Setup packet received"]
    #[inline(always)]
    #[must_use]
    ...
    #[inline(always)]
    #[must_use]
    pub fn endpoint_error(&mut self) -> ENDPOINT_ERROR_W<SIE_STATUS_SPEC> {
        ENDPOINT_ERROR_W::new(self, 23)
    }
    #[doc = "Bit 24 - CRC Error. Raised by the Serial RX engine."]
    #[inline(always)]
    #[must_use]
...
impl crate::Writable for SIE_STATUS_SPEC {
    ...
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0xff8e_1810;
}
